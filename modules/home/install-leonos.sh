...
print_success_banner() {
  echo -e "${GREEN}╔═══════════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${GREEN}║                 leonos Installation Successful!                      ║${NC}"
  echo -e "${GREEN}║                                                                       ║${NC}"
...
print_failure_banner() {
  echo -e "${RED}╔═══════════════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${RED}║                 leonos Installation Failed!                          ║${NC}"
  echo -e "${RED}║                                                                       ║${NC}"
...
print_header "Backup Existing leonos (if any)"

backupname=$(date +"%Y-%m-%d-%H-%M-%S")
if [ -d "leonos" ]; then
  echo -e "${GREEN}leonos exists, backing up to .config/leonos-backups folder.${NC}"
  if [ -d ".config/leonos-backups" ]; then
    echo -e "${GREEN}Moving current version of leonos to backups folder.${NC}"
    mv "$HOME"/leonos .config/leonos-backups/"$backupname"
    sleep 1
  else
    echo -e "${GREEN}Creating the backups folder & moving leonos to it.${NC}"
    mkdir -p .config/leonos-backups
    mv "$HOME"/leonos .config/leonos-backups/"$backupname"
    sleep 1
  fi
else
  echo -e "${GREEN}Thank you for choosing leonos.${NC}"
  echo -e "${GREEN}I hope you find your time here enjoyable!${NC}"
fi

print_header "Cloning leonos Repository"
git clone https://gitlab.com/dwilliam62/leonos.git --depth=1  ~/leonos
cd ~/leonos || exit 1
...
sudo nixos-rebuild boot --flake ~/leonos/#${profile}
